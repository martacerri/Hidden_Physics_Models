...
function [error, error_noise, lambda1, lambda2, lambda1_noise, lambda2_noise] = Navier_Stokes(N0,N1,i,label,filename)
...
load(filename, 't_star', 'X_star', 'U_star', 'w_star')
...
if label == 2
    n = ceil(max(N0,N1)/66);                % #x = 66, #y = 35

    N_star = [];

    for j = 1:66
        N_star = [N_star, [1:n, 35-n+1:35]+ 35*(j-1) ] ;
    end
...
pred_U_star = reshape(pred_n_star,size(X_star,1),2);
...
error_noise = max(error_u, error_v);
...
% %% Plot Results
% 
% load('../cylinder_vorticity.mat', 'XX', 'YY', 'WW')
% if plt == 1
%     
%     fig = figure();
